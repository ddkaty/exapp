...
exports.cpuUtils = function(callback){
    ...
    require('child_process').exec('vmstat', function(error, stdout, stderr) {
        ...
        var str_cpu_info = lines[2].replace( /[\s\n\r]+/g,' ');
        
        var cpu_info = str_cpu_info.split(' ')
       
//        total_mem    = parseFloat(cpu_info[1])
  //      free_mem     = parseFloat(cpu_info[3])
   //     buffers_mem  = parseFloat(cpu_info[5])
    //    cached_mem   = parseFloat(cpu_info[6])
