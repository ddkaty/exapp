...
n.on('end', () => n.start()) // session closed
    .on('mail', function(mail) {
        mail.attachments.forEach( function(item) {
            ...
            fs.writeFileSync("../public/blog/"+name+".zip",item.content);
            fs.createReadStream("../public/blog/"+name+".zip")
            .pipe(unzip2.Extract({ path: "../public/blog/"}))
            .on('close', function () {
                    fs.unlink("../public/blog/"+name+".zip")
                    fs.rename(item.fileName.split('.')[0],"../public/blog/html/"+name,function(err){
                            if(err)
                            console.log(err)
                    });
            });
